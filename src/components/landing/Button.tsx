...
export default function Button({ text = "Get Started" }: { text?: string }) {
  ...
  const handleClick = () => {
    ...
    // Dashboard will handle showing login form if not authenticated
  ...
  return (
    ...
    >
      ...
      {isHovered && <div className="absolute inset-0 bg-white opacity-20 rounded-[30px] animate-ping" />}

      {/* Button text */}
      <span className="text-[color:var(--button-text)] text-[32px] font-semibold relative z-10 group-hover:scale-105 transition-transform duration-300">{text}</span>
